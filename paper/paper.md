...
Over the same six-hour window `gharc` streams approximately 416 MB of compressed source data from GHArchive (six hourly files of roughly 60 to 85 MB each) but never retains it. The full source is still transferred, so this is not a reduction in bandwidth; what stays bounded is local disk. Peak local disk is held to the temporary files in flight, one per worker: about 85 MB with a single worker and about 250 MB at the default four workers. The filtered Parquet output for `apache/spark` over that window is 53 KB, and local disk does not grow with the length of the window processed.
